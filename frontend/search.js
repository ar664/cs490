...
function query() {
    document.getElementById("version").innerHTML = "Version 1.0.2";
    ...
    if(searchKeyword) {
        request += "&question=" + document.getElementById("s").value;
    }
    if(searchTopic) {
...
function examOn() {
    ...
    for(k = 1; k < table.getElementsByTagName("tr").length; k++) {
        table.getElementsByTagName("tr")[k].addEventListener("click", function(){this.classList.toggle("selected")} );
    }
}
function examOff() {
    var table = document.getElementById("queryResults");
    ...
    for(m = 1; m < table.getElementsByTagName("tr").length; m++) {
        table.getElementsByTagName("tr")[m].removeEventListener("click", function(){this.classList.toggle("selected")});
    }
...
function addToExam() {
    ...
    for(s = 0; s < examQuestions.length; s++) {
        var xhttp = new XMLHttpRequest();
        var addRequest = "query=InsertExamQuestion&QuestionID=" + examQuestions[s].id;
        xhttp.onreadystatechange = function() {
            if(this.readyState == 4 && this.status == 200) {
                var echoed = this.responseText;
                var response = JSON.parse(echoed);
